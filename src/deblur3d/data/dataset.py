...
class MultiPageTiffDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if pad_h or pad_w:
            # reflect is invalid if any side pad >= size; fall back to 'replicate'
            pad_mode = "reflect"
            h_in, w_in = vol.shape[1], vol.shape[2]
            if (pad_h // 2) >= h_in or (pad_h - pad_h // 2) >= h_in \
               or (pad_w // 2) >= w_in or (pad_w - pad_w // 2) >= w_in:
                pad_mode = "replicate"   # or "constant", value=0
            ...
            vol = F.pad(
                ...
                (pad_w // 2, pad_w - pad_w // 2,
                 ...
                 0, 0),
                mode=pad_mode,
                value=0.0  # ignored unless mode="constant"
            ).squeeze(0).squeeze(0)
